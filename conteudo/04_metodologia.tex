...
{\color{red} Explicar que o ProteinMPNN foi baixado}
{\color{red} Explicar que o ESMFold é usado via API}
{\color{red} Explicar que o TMScore é USalign}
{\color{red} Explicar que o MDS é sklearn}
{\color{red} Explicar que o PPO é stable baselines}
{\color{red} Explicar que o env é GYM}
...
\begin{algorithm}[H]
  ...
  \State         função de score de conservação $CS$,
  \State         número máximo de repetições do mesmo aminoácido por episódio $N_{rep}$,
  \State         ambiente de aprendizado estágio I $EnvI$,
...
A figura a seguir ilustra uma visão geral da relação entre o ambiente de aprendizagem e o agente durante o primeiro estágio de treinamento.
...
A atualização de $P_{val}$ e $R_{val}$ é realizada com a mesma lógica utilizada em $R_{I}$.
...
\begin{algorithm}[H]
  ...
  \State         sequência inicial $S_{ini}$,
  \State         encoder $Encoder$,
  \State         ambiente de aprendizado $EnvII$,
  \State         tamanho do batch $batch_size$,
  \State         coeficientes da função de recompensa $(a, b, c)$,
  \State         número máximo de repetições do mesmo aminoácido por episódio $N_{rep}$
  \State         Agente pré treinado \textit{GenSeq}
  \Ensure Agente treinado \textit{GenSeq}
  \State $env \gets EnvII(S_{ini}, Encoder, ESMFold, TMScore, M_{max}, N_{rep})$ 
  \State $S_{enc} \gets Encoder(S_{ini})$ 
  \State $R_{hist} \gets []$ 
  \For{$i = 1$ to $N_{iter}$}
      \State $(p,r) \gets GenSeq(S_{enc})$ 
      \State Inserir $r$ na posição $p$ de $S$
      \State $(S_{enc}, R_{I}, done) \gets env.step(p, r)$ 
      \State Adiciona $R_{I}$ ao $R_{hist}$ 
      \If{done = True}
          \State $env.reset()$ 
...
{\color{red} Pseudo código}
{\color{red} Instruções de replicabilidade}
